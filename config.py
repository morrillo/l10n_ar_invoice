...
class l10n_ar_invoice_del_journal(osv.osv_memory):
	...
	# import pdb;pdb.set_trace()
        try:
            ...
            _logger.info('Deleted journals ')
        except Exception, e:
            raise osv.except_osv(_('Ilegal Operation'),
                                 _('You want remove journals with moves'))
...
class l10n_ar_invoice_new_sequence(osv.osv_memory):
    ...
    _defaults = {
	'name': 'Sequence l10n_ar_invoice',
	'code': 'journal_sale_vou'
	}
...
class l10n_ar_invoice_config(osv.osv_memory):
	...
	# import pdb;pdb.set_trace()
        ...
        data = self.read(cr, uid, ids, ['company_id',
                                        ...
                                        'point_of_sale',
                                        'purchase_by_class'])
	company_id = data[0]['company_id'][0]
	do_export = data[0]['do_export']
	responsability_id = data[0]['responsability_id'][0]
	remove_old_journals = data[0]['remove_old_journals']
	point_of_sale = data[0]['point_of_sale']

	jou, seq = self.update_new_journals (cr, uid, ids, company_id,
                                         responsability_id, do_export,
                                         remove_old_journals, point_of_sale)
	...
	jou, seq = self.update_new_journals (cr, uid, ids, company_id,
        ...
        #                                'journal_class_id', 'point_of_sale', 'sequence_name',
        #                                'currency'])
        names = [ j['name'] for j in jou ]
        for j in jou:
	    val = {}
            val['sequence_id'] = obj_sequence.search(cr, uid, [('name','=',j['sequence_name'])]).pop()
	    val['code'] = j['name'][j['name'].find('(')+1:j['name'].find(')')]
	    val['name'] = j['name']
	    val['type'] = j['type']
	    val['company_id'] = company_id
	    val['journal_class_id'] = j['journal_class_id']
	    val['point_of_sale'] = point_of_sale
	    val['sequence_name'] = j['sequence_name']
	    if j.has_key('currency'):	
		    val['currency'] = j['currency']
            # val = dict( (k, v[0]) if type(v) is tuple else (k, v) for k,v in val.items() )
	    # import pdb;pdb.set_trace()
            jid = obj_journal.create(cr, uid, val)
        _logger.info('Journals created %s' % ','.join(names))
	return 
